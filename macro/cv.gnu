...
set ylabel "CV: 1/F^{2} [10^{12} C^{-2}]" 
set xlabel "U [V]" offset first 565, char 1.5

plot [-10:1100][0:*] for [file in files ] file  u (abs(column("bias"))):((abs(column("CV455"))*1e12)) w p ls 1 t '455 Hz, '.columnhead(columnLegend), \
for [file in files ] file  u (abs(column("bias"))):((abs(column("CV1M"))*1e12)) w p ls 3 t '1 MHz, '.columnhead(columnLegend)
