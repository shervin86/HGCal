{
  ...
  TMultiGraph *sp_example=(TMultiGraph*)_file0->Get("refP1030_spectra");
  TMultiGraph *sp_example2=(TMultiGraph*)_file0->Get("refP1930_spectra");
  TMultiGraph *spirr_example2=(TMultiGraph*)_file0->Get("irrP2230_spectra");
  ...
  TMultiGraph *iv_30 = PlotAllIV(c1, _file0, "30_iv","p");
  TMultiGraph *iv_20 = PlotAllIV(c1, _file0, "20_iv","l");
  TMultiGraph *ivAll = new TMultiGraph();
  ivAll->Add(iv_30);
  ivAll->Add(iv_20);
  ivAll->Draw("A");
  c1->BuildLegend(0.82,0.2,1,1);
  ivAll->GetXaxis()->SetTitle(iv_30->GetXaxis()->GetTitle());
  ivAll->GetYaxis()->SetTitle(iv_30->GetYaxis()->GetTitle());

}
  basP1020x0_allSpectra->Draw("A");
  TLegend *leg =c1->BuildLegend(0.85,0.,1,1);
  leg->SetBorderSize(0);
  c1->SaveAs(outDir+"baseline-example.png");
  basP1020x0_allSpectra->GetXaxis()->SetRangeUser(startSignal, endSignal);
  c1->SaveAs(outDir+"baseline-example-zoom.png");

  basP1030_baselinesAverage->Draw("A");
  c1->SaveAs(outDir+"baselineAverage-example.png");

  checkBaselines_m30->Draw("A");
  leg = c1->BuildLegend(0.85,0.7,1,1);
  leg->SetBorderSize(0);
  ...
  sp_example->GetXaxis()->SetRangeUser(0.8e-7, 0.88e-7);
  sp_example->GetYaxis()->SetRangeUser(-0.2e-3, 0.2e-3);
  sp_example->Draw("A");
  line.Draw();
  c1->SaveAs(outDir+"spectra-example_pre_zoom.pdf");

  // long tail at low bias voltage (slow)
  // fast signal is below baseline!
  sp_example->GetXaxis()->SetRangeUser(0.9e-7, 1.1e-7);
  sp_example->GetYaxis()->SetRangeUser(-0.1e-3, 0.4e-3);
  TLine line2(0.9e-7, 0, 1.1e-7,0);
  line2.SetLineWidth(2);
  sp_example->Draw("A");
  line2.Draw();
  c1->SaveAs(outDir+"spectra-example_post_zoom.pdf");

  //------------------------------
  // need to verify the correctness of the baseline subtraction!
  sp_example2->GetXaxis()->SetRangeUser(0.8e-7, 0.88e-7);
  sp_example2->GetYaxis()->SetRangeUser(-0.2e-3, 0.2e-3);
  sp_example2->Draw("A");
  line.Draw();
  c1->SaveAs(outDir+"spectra-example2_pre_zoom.pdf");

  // long tail at low bias voltage (slow)
  // fast signal is below baseline!
  sp_example2->GetXaxis()->SetRangeUser(0.9e-7, 1.1e-7);
  sp_example2->GetYaxis()->SetRangeUser(-0.1e-3, 0.4e-3);
  sp_example2->Draw("A");
  line2.Draw();
  c1->SaveAs(outDir+"spectra-example2_post_zoom.pdf");

  //------------------------------
  // need to verify the correctness of the baseline subtraction!
  spirr_example2->GetXaxis()->SetRangeUser(0.8e-7, 0.88e-7);
  spirr_example2->GetYaxis()->SetRangeUser(-0.2e-3, 0.2e-3);
  spirr_example2->Draw("A");
  line.Draw();
  c1->SaveAs(outDir+"spectraIrr-example2_pre_zoom.pdf");

  // long tail at low bias voltage (slow)
  // fast signal is below baseline!
  spirr_example2->GetXaxis()->SetRangeUser(0.9e-7, 1.1e-7);
  spirr_example2->GetYaxis()->SetRangeUser(-0.1e-3, 0.4e-3);
  spirr_example2->Draw("A");
  line2.Draw();
  c1->SaveAs(outDir+"spectraIrr-example2_post_zoom.pdf");


  std::vector<TMultiGraph*> cce_20=GetCCEs(_file0,"20_CCEvsV");
  std::vector<TMultiGraph*> cce_30=GetCCEs(_file0,"30_CCEvsV");


  //  CCE_vs_V(c1, refcce20, cce20);
  CCE_vs_V(c1, cce_30);
  c1->SaveAs(outDir+"CCE_30.pdf");

  CCE_vs_V(c1, cce_20);
  c1->SaveAs(outDir+"CCE_20.pdf");

  // std::vector<TMultiGraph*> m_30 = SplitCCEs(cce_30);
  // for(auto itr=m_30.begin(); itr!=m_30.end(); itr++){
  //   TMultiGraph *m = *itr;
  //   m->Draw("A");
...
((TGraph *)irrN100_CCEvsV->GetListOfGraphs().At(0)).SetMarkerColor(ci);
  ...
  refN100_spectra->GetYaxis()->SetRangeUser(-0.1e-3, 0.4e-3);
  TLine line2(0.9e-7, 0, 1.1e-7,0);
  line2.SetLineWidth(2);
  refN100_spectra->Draw("A");
  line2.Draw();
