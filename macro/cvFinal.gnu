...
if(ARGC==1){
p [2e14:1.8e16][0:1.1] file u "fluence":((abs(column("CV"))*1e6))  w p ls 1 title column(columnLabel)
}else{
p [2e14:1.8e16][0:*] file u "fluence":((abs(column("CV"))*1e6))  w p ls 1 title column(columnLabel), file2 u "fluence":((abs(column("CV"))*1e6))  w p ls 3 title column(columnLabel)
}
@OUTPUT 'CV.pdf'
