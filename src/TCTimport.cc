#include "TCTimport.h"
bool TCTimport::ReadLine(std::ifstream& f, TCTspectrum &meas){
    float value; 
    std::string line;
    for(int i=0; i<4; i++){ // skip fields CH#, X-Position [um], Y-Position [um], Z-Position [um],
      f >> line; // 
    }
    
    if(f.fail()) return false; // check the end of the file
    f >> value;  // bias
...
TCTspectrumCollection_t TCTimport::ImportFromFile(std::string filename){
